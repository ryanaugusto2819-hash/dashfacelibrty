    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-[1440px] mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          ...
          <KPICard
            title="Custo / Lead"
            value={`R$ ${fmt(kpi.costPerLead)}`}
            icon={Target}
            trend="3.1%"
            trendUp={false}
            glowClass="metric-glow-orange"
            gradientClass="bg-warning"
          />
          <KPICard
            title="Custo / Venda"
            value={`R$ ${fmt(kpi.costPerSale)}`}
            icon={ShoppingCart}
            trend="5.2%"
            trendUp={false}
            glowClass="metric-glow-purple"
            gradientClass="bg-[hsl(280_65%_60%)]"
          />
          <KPICard
            title="Faturamento"
            value={`R$ ${fmt(kpi.totalRevenue)}`}
            icon={TrendingUp}
            trend="15.7%"
            trendUp
